...
Afterwards rebar (the rust-regex benchmarking suite) was extended to include engines which would be configured to our wanted four strategies. Changes can be found in the [LindenRegex fork on the branch "mw/prefix-acc-cmp"](https://github.com/LindenRegex/rebar/tree/mw/prefix-acc-cmp). This was done by extending the existing "regex-automata" engine which benched individual regex engines of rust-regex (dfa, pikevm, onepass, backtracking, etc.). The `rust-regex` dependency was rewired to point to our fork with the implemented `PrefilterStrategy`. Additional targets ("pikevm/noAcc", "pikevm/accOnce", "pikevm/accEmptyState", "pikevm/accOneAhead") were added. Then, these targets were added to _all_ existing benchmarks and run on a silent system. The benchmarking result were [committed to the rebar fork](https://github.com/LindenRegex/rebar/blob/mw/prefix-acc-cmp/prefilters.csv).
