...
To guide heuristics, we define a configuration type which holds parameters which influence the decisions made by the Meta engine. For now, we only store the memory limit we wish to impose on the engines during runtime. If an engine can potentially go over that limit, it will not be considered for execution. If no memory limit is provided, it is assumed that there is no limit on the memory usage. Expressing a memory limit is difficult especially in a system such as Rocq where the size of values is not precisely defined and it may also depend on the language to which we extract the Rocq code to. Hence, here we only hope to try to respect the memory limit as precisely as possible. We express this limit with a natural number $n$ and interpret is as the memory occupied by $n$ pointer-sized values. We additionally define the estimated peak usage of the MemoBT as $|r| dot |s|$ for a regex $r$ and a haystack $s$.

#linden-listing("Engine/Meta/Meta.v", (
  "meta_config",
  "memobt_peak_memory_usage",
))[The definition of the Meta engine config and the estimate of MemoBT's memory usage.]
...
The correctness of follows directly from the correctness of the individual engines. The heuristic of picking an engine does not affect correctness; regardless which engine was picked, the engine will produce a correct result. Heuristics affect only the practical aspect of performing matching. We state this correctness theorem in @thm:meta-anchored-correctness. For any config, supported regex, and haystack, running the anchored Meta engine produces the same result as defined by the formal semantics of backtracking trees. ```rocq meta_supported_regex``` accepts exactly the same regexes as the PikeVM and the MemoBT. Thus, this Meta anchored matching function is another instance of the ```rocq AnchoredEngine``` typeclass#note[Proven in #source-permalink(find-statement("Engine/Meta/Meta.v", "MetaSearchAnchored")).].
...
This function first tries to dispatch matching methods that do not require running a full regex engine. As such, it first attempts to find the match using literal optimizations from @lst:try-lit-search. For lack of a better instance at the moment of writing, we use the naive brute-force substring search algorithm defined in @lst:naive-substring-search. If this attempt is unable to find a match, we reach for the anchored search optimization from @lst:try-anchored-search. The anchored engine used here is the anchored Meta engine defined in @lst:meta-anchored-engine. If this as well does not yield a match, we finally succumb to using full unanchored engines. We again use the provided memory limit configuration to decide which engine to run. If the memory limit allows for running the MemoBT engine, we do so by means of the unanchoring technique from @lst:unanchor-engine. This means we run the anchored MemoBT with the lazy prefix prepended to the regex. For the MemoBT we must additionally provide a proof that it indeed supports regexes with the lazy prefix. If the memory limit could potentially be exceeded, we use the prefix-accelerated unanchored PikeVM from @sec:unanchored-pikevm. We conclude with proving the correctness of this unanchored Meta engine in @thm:meta-unanchored-correctness. For any config, supported regex, and haystack, running the unanchored Meta engine produces the same result as defined by the formal semantics of backtracking trees for a regex with the lazy prefix. This, of course, is another instance of an ```rocq UnanchoredEngine```#note[Proven in #source-permalink(find-statement("Engine/Meta/Meta.v", "MetaEngine")).].

#linden-theorem("Engine/Meta/Meta.v", "search_correct", proof: [
  By @thm:try-lit-search-correctness, @thm:try-anchored-search-correctness, and the correctness of the individual engines and the unanchoring construction.
