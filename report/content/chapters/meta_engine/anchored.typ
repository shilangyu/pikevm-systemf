...
We now define the anchored optimization in @lst:try-anchored-search. Given a regex, a haystack, and an anchored engine, if possible we return the result of *unanchored* matching. Other than checking if the regex is anchored, we additionally check if the passed haystack is set to the beginning position. If it is, we return the result obtained from running the anchored engine. If it is not at the beginning position, we immediately know matching would fail and thus return ```rocq Ok None```.
...
Then, we say that given an anchored list of actions and a haystack at a position different from the beginning, no match can exist. This is formalized in @thm:anchored-match-not-begin. Recall that in Linden, inputs (which represent haystacks along with the current positions) are formed from a string of future characters and a string of past characters. So to say that a haystack is set to a position different from the beginning, it is the same as saying the string of past characters is non-empty. We represent it here by explicitly forming the Linden input type with the past characters being constructed by the List's ```rocq cons``` (```rocq ::```) constructor which is by definition non-empty.
...
And we conclude with the correctness theorem of ```rocq try_anchored_search``` in @thm:try-anchored-search-correctness. If ```rocq try_anchored_search``` returns ```rocq Ok```, then the contained result corresponds exactly to the result defined by the backtracking tree semantics of the regex with a lazy prefix. We are not interested in the case where ```rocq try_anchored_search``` returns ```rocq Unsupported``` as that indicates that no optimization was possible.
