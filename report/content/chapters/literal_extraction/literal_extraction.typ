...
To get the proof of correctness for the prefix acceleration optimization we must first formalize this constant-strings-in-regex analysis, called _literal extraction_ (@sec:literals). Then we formalize the notion of looking for those literals in a haystack (@sec:substring-search). These formalizations allows us to prove the correctness of literal extraction (@sec:literal-extraction-correctness). And finally, we tackle the prefix acceleration proof together with the peculiarities of the PikeVM engine (@sec:prefix-acceleration).
