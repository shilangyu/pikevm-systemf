...
We must additionally define two important operators on literals. The first one is _chaining_ defined in @lst:chain-merge-literals. It computes the literal resulting from joining two immediately consecutive literals, most commonly from a sequence. If any of the inputs is ```rocq Impossible```, the result is also ```rocq Impossible```. The intuition is that if one of the literals says that no match can exist, then adding anything to the left or right of that does not change this fact. Naturally, chaining is not commutative, it is akin to string concatenation. If the left operand is ```rocq Prefix```, then the right operand cannot be used because there might be some unknown characters in between. The second operator is _merging_ defined in @lst:chain-merge-literals. It computes the result of joining two overlapping literals, most commonly from a disjunction. If either of the literals is an ```rocq Impossible```, then the result is that equal to the other literal. If the literals are the same, the result is that literal. Otherwise we must degrade into a ```rocq Prefix``` with the string that is the longest common prefix of both. We can see that in both operators ```rocq Exact``` literals are quite brittle. To get an ```rocq Exact``` literal out of the operators, a ```rocq Prefix``` literal cannot be present. This is expected, as ```rocq Exact``` literals encode a very strong property about matches.
