...
To get started we describe what it means for a string to start with a different string. For this we define the ```rocq starts_with``` inductive seen in @lst:starts-with. We say that "$s s$ is the prefix of $s$" to mean that $s$ starts with $s s$, or more precisely ```rocq starts_with ss s```. The definition has a base case stating that an empty string is the prefix of any string, and an inductive case stating that if both strings start with the same character and the rest of the first string is the prefix of the rest of the second string, then the first string is the prefix of the second string. One can easily see that this definition is decidable#note[Proven in #linden-permalink(linden-statement("Engine/Prefix.v", "starts_with_dec"))] and that it defines a @preorder:intro on strings#note[Proven in #linden-permalink(linden-statement("Engine/Prefix.v", "StartsWithPreOrder"))].
