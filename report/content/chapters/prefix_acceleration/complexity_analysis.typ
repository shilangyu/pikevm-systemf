...
#let ssearch-def = $ssearch : sans("string") times sans("string") -> sans("option") NN$


Let $s s$ be a string and $s$ be a haystack. Let #ssearch-def be a substring search algorithm such that $ssearch s s" "s$ returns the index of the first occurrence of the substring $s s$ within $s$. If no occurrence exists, it returns ```rocq None```.
...
The progressing search property ensures that each #ssearch call makes progress in the haystack and thus #ssearch does not explore the same part of the haystack more than once. Consider the following example trace of a progressing search algorithm $T$. We #underline[underline] the haystack portion on which #ssearch can no longer be called on due to the progression property, and mark the haystack position on which #ssearch is called.

$
                                          & : T "is initialized with the haystack" #hay[hello world] \
           #hay(position: 0)[hello world] & : T "calls" #ssearch "at position 0 with substring" #hay[lo] -> 3 \
               #hay(seen: 4)[hello world] & : T "does some work which advances the haystack position by 1" \
  #hay(seen: 4, position: 5)[hello world] & : T "calls" #ssearch "at position 5 with substring" #hay[o] -> 2 \
  #hay(seen: 8, position: 8)[hello world] & : T "calls" #ssearch "at position 8 with substring" #hay[rld] -> 0 \
