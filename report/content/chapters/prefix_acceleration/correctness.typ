...
We first discuss the technical details that allowed to modify the anchored PikeVM definition to support unanchored matching without losing any of the previous functionality. The PikeVM's state is extended with a single optional parameter which we call ```rocq nextprefix```. It stores three things: the literal $ell$ extracted from the regex, a counter indicating in how many characters the prefix of that literal will match the haystack, and an instance of a substring search algorithm conforming to @lst:substring-search-class. If this triple is not set, the PikeVM will operate in the original anchored mode. The original small-step semantics of the PikeVM#note[Defined in #linden-permalink(linden-statement("Engine/PikeVM.v", "pike_vm_step"))] are modified to take this new state parameter into account. Most of them remain unchanged, but instead new small-step rules are added to perform the work needed to, 1) simulate the lazy prefix#note[`pvs_nextchar_generate`], 2) perform the filtering optimization#note[`pvs_nextchar_filter`], 3) perform the acceleration optimization#note[`pvs_acc`]. Below we explain which existing rules needed adjustments.
...
The new unanchored PikeVM has an executable functional version#note[Defined in #linden-permalink(linden-statement("Engine/FunctionalPikeVM.v", "pike_vm_match_unanchored"))] which is proven to follow exactly the small-step semantics#note(text(0.95em)[Proven in #linden-permalink(linden-statement("Engine/FunctionalPikeVM.v", "pike_vm_match_correct_unanchored"))]).
