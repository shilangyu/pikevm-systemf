...
- *Epsilon $epsilon$* -- the regex that matches the empty string. The ECMAScript regex ```re /|abc/``` is equivalent to $epsilon|"abc"$ in Linden.
...
- *Non-capturing group $chevron.l r chevron.r$* -- A grouping construct that does not create a capture. The ECMAScript regex ```re /(?:abc)/``` is equivalent to $chevron.l "abc" chevron.r$ in Linden.
...
- *Anchor $a$* -- Specifies a position in the haystack rather than characters. It is merely an assertion, it does not consume characters. The ECMAScript regex ```re /^abc$/``` is equivalent to $\^"abc"\$$ in Linden.
