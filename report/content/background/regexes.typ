...
==== Anchor $a$
Specifies a position in the haystack rather than characters. It is merely an assertion, it does not consume characters. ```re ^``` asserts that we are at the start of the haystack, and ```re $``` asserts that we are at the end of the haystack. ```re \b``` asserts that we are at a word boundary and ```re \B``` that we are not. A word boundary is a position in the haystack where exactly one of the two adjacent characters is a word character and the other character is not, for example ```re /\ba\Bf \bc/``` matches #hay[af c]. The ECMAScript regex ```re /^abc$/``` is equivalent to $\^"abc"\$$ in Linden.
