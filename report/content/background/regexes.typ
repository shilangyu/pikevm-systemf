  ...
  columns(2)[
    ...
    #bnf(
      Prod($c d$, {
        ...
        Or[$dot$][Dot]
        Or[$esc("w")$][$esc("W")$][$esc("d")$][$esc("D")$][$esc("s")$][$esc("S")$][$esc("p"){"property"}$][$esc("P"){"property"}$][Character class]
        Or[$[\^c d]$][Inversion]
        Or[$[\^]$][All]
        Or[$[thin]$][Empty]
      }),
...
- *Character descriptor $c d$* -- Describe sets of characters. If the match something in the haystack, they will match exactly one character. They can be single literal characters, ranges of characters, unions of character descriptors, the dot (which matches any character except line terminators), character classes (such as word characters, digits, whitespace, Unicode properties), inversion of character descriptors, the set of all characters, or the empty set (it never matches). For instance, the ECMAScript regex ```regex [a-c0-7]\d.``` is equivalent to $[range("a", "c")[range("0", "7")]]#h(0pt)esc("d") dot$ in Linden. For brevity, we will flatten descriptors in $[thin]$, giving us $[range("a", "c")range("0", "7")]$ instead of $[range("a", "c")[range("0", "7")]]$
